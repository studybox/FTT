...
from commonroad.scenario.obstacle import DynamicObstacle, StaticObstacle, ObstacleRole, ObstacleType
...
def line_marking_to_linestyle(line_marking:LineMarking) -> Tuple:
    ...
    return {LineMarking.NO_MARKING: (None, (None,None), 0.0,),
    ...
    for i_lanelet, lanelet in enumerate(lanelets):
        ...
        if draw_border_vertices or draw_right_bound:
            ...
            if draw_line_markings and lanelet.line_marking_right_vertices is not None:
                #print(lanelet.lanelet_id, lanelet.line_marking_right_vertices)
...
def draw_dynamic_obstacles(obj: Union[List[DynamicObstacle],DynamicObstacle],
    ...
    def collecting(o: DynamicObstacle):
        ...
        if draw_icon and (o.obstacle_type == ObstacleType('car') or o.obstacle_type == ObstacleType('bus') or o.obstacle_type == ObstacleType('truck')): #and type(o.prediction) == commonroad.prediction.prediction.TrajectoryPrediction:
            if time_begin == 0:
            ...
            if inital_state is not None:
                draw_car(inital_state.position[0], inital_state.position[1], inital_state.orientation, 2.5*o.obstacle_shape.length/4.2, 2.5*o.obstacle_shape.width/2.0,
                         ax, zorder=30)
    ...
    tmp_array  = np.array(list(map(collecting,obj)), dtype=object)
...
def draw_car(pos_x: Union[int,float], pos_y: Union[int,float], rotate: Union[int,float],
             scale_h: Union[int,float],scale_w: Union[int,float],  ax: mpl.axes.Axes, zorder: int = 5, carcolor: str ='#ffffff',
             lw=0.5):
    rotate = rotate + np.pi
    ...
    def transform(vertices, pos_x, pos_y, rotate, scale_h, scale_w):
        ...
        scalemat = np.array([[scale_h*1/356.3211, 0, 0], [0, scale_w*1/356.3211, 0],
                             [0, 0, 1]])
    ...
    verts1 = transform(verts1, pos_x, pos_y, rotate, scale_h, scale_w)
    verts2 = transform(verts2, pos_x, pos_y, rotate, scale_h, scale_w)
    verts3 = transform(verts3, pos_x, pos_y, rotate, scale_h, scale_w)
    verts4 = transform(verts4, pos_x, pos_y, rotate, scale_h, scale_w)
    verts5 = transform(verts5, pos_x, pos_y, rotate, scale_h, scale_w)
    verts6 = transform(verts6, pos_x, pos_y, rotate, scale_h, scale_w)
    verts7 = transform(verts7, pos_x, pos_y, rotate, scale_h, scale_w)
    verts8 = transform(verts8, pos_x, pos_y, rotate, scale_h, scale_w)
