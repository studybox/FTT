...
class AttDest(nn.Module):
    def __init__(self, n_agt: int):
        ...
        self.dist = nn.Sequential(
            nn.Linear(2, n_agt),
            nn.ReLU(inplace=True),
            NormLin(n_agt, n_agt, norm=norm, ng=ng),
        )

        self.agt = NormLin(2 * n_agt, n_agt, norm=norm, ng=ng)
...
class AttTraj(nn.Module):
    def __init__(self, config):
        super(AttTraj, self).__init__()
        norm = "GN"
        ng = 1
        ...
        n_agt = config["n_actor"]
        lateral = []
        for i in range(len(n_out)):
            lateral.append(Conv1d(n_out[i], n_agt, norm=norm, ng=ng, act=False))
        self.lateral = nn.ModuleList(lateral)

        self.output = Res1d(n_agt, n_agt, norm=norm, ng=ng)

        #self.agt = NormLin(2 * n_agt, n_agt, norm=norm, ng=ng)
        self.agt = nn.Linear(2 * n_agt, n_agt)

    def forward(self, agts: Tensor, agt_ctrs: Tensor, pred_ctrs: Tensor) -> Tensor:
        n_agt = agts.size(1)
        num_mods = pred_ctrs.size(1)
        agt_ctrs = agt_ctrs.unsqueeze(1).repeat(1, num_mods, 1)
        #print(agt_ctrs.size(), pred_ctrs.size())
        #dist = (agt_ctrs.unsqueeze(1) - dest_ctrs).view(-1, 2)
        pred_ctrs = torch.cat([agt_ctrs.unsqueeze(2), pred_ctrs], dim=2)
        #print(pred_ctrs.size())
        preds = []
        for m in range(num_mods):
            out = pred_ctrs[:, m].permute(0, 2, 1)
            outputs = []
            for i in range(len(self.groups)):
                out = self.groups[i](out)
                outputs.append(out)
            ...
            preds.append(self.output(out)[:, :, -1])

        preds = torch.stack(preds, 1)
        #print(preds.size())
        agts = agts.unsqueeze(1).repeat(1, num_mods, 1).view(-1, n_agt)

        agts = torch.cat((preds.view(-1, n_agt), agts), 1)
        agts = self.agt(agts)
        return agts
...
class ScoreNet(nn.Module):
    def __init__(self, config):
        ...
        #            LinRes(n_actor, n_actor, norm=norm, ng=ng),
        #            nn.Linear(n_actor, 2 * config["num_preds"]),
        #        )

        self.att_dest = AttTraj(config)#AttDest(n_actor)
    ...
    def forward(self, actors: Tensor, actor_idcs: List[Tensor], actor_ctrs: List[Tensor], preds: List[Tensor]) -> Dict[str, List[Tensor]]:

        start_pos = torch.cat(actor_ctrs, 0)
        pred = torch.stack(preds, 1)
        reg = torch.stack([(torch.cumsum(x, dim=1) + start_pos.unsqueeze(1)) for x in preds], 1)
        # (a, m, p, 2)
        ...
        sorted_cls, sort_idcs = cls.sort(1, descending=True)
        row_idcs = torch.arange(len(sort_idcs)).long().to(sort_idcs.device)
        row_idcs = row_idcs.view(-1, 1).repeat(1, sort_idcs.size(1)).view(-1)
        sort_idcs = sort_idcs.view(-1)
        reg = reg[row_idcs, sort_idcs].view(sorted_cls.size(0), sorted_cls.size(1), -1, 2)
        pred = pred[row_idcs, sort_idcs].view(sorted_cls.size(0), sorted_cls.size(1), -1, 2)

        out = dict()
        out["cls"], out["reg"], out["pred"], out["cls_unsorted"] = [], [], pred[:, 0], cls
        for i in range(len(actor_idcs)):
            idcs = actor_idcs[i]
            ctrs = actor_ctrs[i].view(-1, 1, 1, 2)
            out["cls"].append(sorted_cls[idcs])
            out["reg"].append(reg[idcs])
        out["cls_sorted"] = sorted_cls
        out["sort_idcs"] = sort_idcs
        out["row_idcs"] = row_idcs
        return out