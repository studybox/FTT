...
class OTPlanSampler:
    ...
    def __init__(
        ...
        if method == "exact":
            ...
            self.ot_fn = partial(pot.partial.entropic_partial_wasserstein, reg=reg, verbose=True, log=True)
        else:
    ...
    def get_map(self, x0, x1, a=None, b=None, m=None):
        ...
        if self.normalize_cost:
            ...
            M = torch.log(1 + M) / np.log(1 + 1000)
        if m is None:
            p = self.ot_fn(a, b, M.detach().cpu().numpy())
        else:
            p, log = self.ot_fn(a, b, M.detach().cpu().numpy(), m=m)
    ...
    # def sample_plan_with_targets(self, x0, x1, replace=True):


    def sample_plan_with_labels(self, x0, x1, y0=None, y1=None, replace=True, num_samples=None):
        r"""Compute the OT plan $\pi$ (wrt squared Euclidean cost) between a source and a target
        ...
        """
        pi = self.get_map(x0, x1)
        num_samples = x0.shape[0] if num_samples is None else num_samples
        i, j = self.sample_map(pi, num_samples, replace=replace)
        return (
            x0[i],
            x1[j],
