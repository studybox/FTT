...
class ManeuverLoss(nn.Module):
    def __init__(self, config):
        super(ManeuverLoss, self).__init__()
        self.config = config
        self.cls_loss = nn.CrossEntropyLoss(reduction="mean")

    def forward(self, logits, dists):
        idcs = torch.argmin(dists, dim=-1)
        return self.cls_loss(logits, idcs)
